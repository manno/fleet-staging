...
func NewIgnoreNormalizer(ignore []ResourceIgnoreDifferences, overrides map[string]ResourceOverride) (diff.Normalizer, error) {
	for key, override := range overrides {
		...
		if len(override.IgnoreDifferences.JSONPointers) > 0 {
			ignore = append(ignore, ResourceIgnoreDifferences{
				Group:        group,
				Kind:         kind,
